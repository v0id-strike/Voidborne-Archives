...
| Open Shortest Path First                          | `OSPF`      | Bu Internet protokoli (IP) tarmog'idagi yagona avtonom tizim (AS) ichidagi trafikni yo'naltirish uchun ichki shlyuz protokoli (IGP).                                                                                                                                                                 |
...
| Microsoft Baseline Security Analyzer              | `MBSA`      | Bu Windows kompyuterlari, tarmoqlari va tizimlarida potentsial xavfsizlik zaifliklarini aniqlash uchun foydalaniladigan Microsoft-ning bepul xavfsizlik vositasidir.                                                                                                                                 |
...
| Network File System                                       | `NFS`        | `111`, `2049`    | Masofaviy tizimlarni o'rnatish uchun ishlatiladi                                                                                                                                                    |
...
| Internet Security Association and Key Management Protocol | `ISAKMP`     | `500`            | VPN ulanishlari uchun ishlatiladi                                                                                                                                                                   |
...
| MySQL                               | `MySQL`      | `3306`      | Bu ochiq manbali ma'lumotlar bazasini boshqarish tizimi.                                                                                                |
...
Shuning uchun, simsiz tarmoqqa ulanish uchun qurilma tarmoq doirasida bo'lishi va tarmoq nomi va parol kabi to'g'ri tarmoq sozlamalari bilan sozlangan bo'lishi kerak. Ulangandan so'ng, qurilmalar bir-biri va Internet bilan aloqa o'rnatishi mumkin, bu foydalanuvchilarga onlayn resurslarga kirish va ma'lumotlar almashish imkonini beradi.
...
| `Xavfsizlik xususiyatlari`  | Tarmoq resurslariga kirishni nazorat qilish va xavfsizlik tahdidlarining oldini olish uchun foydalaniladigan [Kirishni boshqarish roʻyxatlari](https://en.wikipedia.org/wiki/Access-control_list) (`ACL') kabi.                                                   |
...
"VLAN" dan foydalanishda ko'plab afzalliklarga erishiladi, jumladan:
...
Endi biz “VLAN” interfeysini yaratadigan jismoniy “Ethernet” interfeysining nomini topishimiz kerak, bu “eth0”:
```bash
...
Biz [vlan](https://www.wireshark.org/docs/dfref/v/vlan.html) filtri yordamida `Wireshark` bilan tarmoqdagi `VLAN` teglangan trafikni aniqlashimiz va tahlil qilishimiz mumkin. Misol uchun, tarmoq paketi dempini tahlil qilganda, biz `vlan` filtri yordamida `802.1Q` tegi bilan paketlarni tekshirishimiz mumkin:
...
“VLAN sakrash” hujumlarini amalga oshirish uchun [Yersinia](https://linux.die.net/man/8/yersinia) kabi vositalardan foydalanishimiz mumkin:
...
### VXLAN
Yuqorida aytib o'tgan edikki, "Ethernet" ramkasi ichidagi "802.1Q" sarlavhasidagi "VID" maydoni atigi 12 bit bo'lib, 4094 VLAN-ga ruxsat beradi. Ushbu VLAN soni kichik tarmoqlar uchun etarli bo'lishi mumkin bo'lsa-da, keng segmentatsiyani talab qiladigan ma'lumotlar markazlari va bulutli xizmat ko'rsatuvchi provayderlar uchun ko'proq talab qilinadi. Bunga qo'shimcha ravishda, joriy Layer 2 tarmoqlari ortiqcha yo'llardan kelib chiqadigan tarmoq tsikllarining oldini olish uchun "IEEE 802.1D" "Spanning Tree Protocol" ('STP') dan foydalanadi. Biroq, ba'zi ma'lumotlar markazi operatorlari "STP" bilan bog'liq cheklovlarga duch kelishadi, masalan, ulanishni bloklash, bu mavjud portlarni kamaytiradi va ko'p yo'l orqali chidamlilikni oldini oladi. Ushbu muammolar Layer 2 jismoniy infratuzilmasiga tayanadigan virtuallashtirilgan muhitlarda tarmoq samaradorligiga to'sqinlik qiladi. Bunday muhitlarda muhim talab - 2-qavat tarmog'ining butun ma'lumotlar markazi bo'ylab va hatto ma'lumotlar markazlari o'rtasida hisoblash, tarmoq va saqlash resurslarini samarali taqsimlash uchun uzluksiz kengaytirilishi. Shunga qaramay, “STP” kabi anʼanaviy yondashuvlar halqasiz topologiyani taʼminlab, koʻplab havolalarni oʻchirib qoʻyishi mumkin va bu muammoni yanada kuchaytiradi.

[RFC7348](https://datatracker.ietf.org/doc/html/rfc7348) 2-qavatdagi tarmoqlardagi ushbu muammolar va cheklovlarga yechimni `Virtual kengaytiriladigan mahalliy tarmoq` (`VXLAN`) ni joriy etish orqali taklif qiladi, bu asosan `qatlam 2-qatlam sxemasi` hisoblanadi. `VXLAN` an'anaviy 2-qavat tarmoqlarining cheklovlarini hal qilish va virtual mashinalar (VM) bilan ko'p ijarachili muhitda Layer 2 va Layer 3 ma'lumotlar markazi tarmoq infratuzilmalari talablariga javob berish uchun maxsus ishlab chiqilgan. Mavjud tarmoq infratuzilmasi orqali ishlaydigan `VXLAN` 2-qatlam tarmog'ini muammosiz kengaytirishning innovatsion usulini taqdim etadi. Uning asosiy maqsadi 2-qavat tarmoqlarini keng ko'lamli ma'lumotlar markazi landshaftlari bo'ylab masshtablashni osonlashtirish, hattoki bir nechta jismoniy ma'lumotlar joylarini qamrab olishdir. Har bir “VXLAN” qoplamasi “VXLAN segmenti” deb nomlanadi, bu faqat bir xil VXLAN segmentidagi VM’lar bir-biri bilan bog‘lanishini ta’minlaydi va shu bilan tarmoq izolyatsiyasi va xavfsizligini ta’minlaydi. `VXLAN tarmoq identifikatori` (`VNI`) deb nomlanuvchi 24-bitli segment identifikatori har bir VXLAN segmentini noyob tarzda aniqlaydi. VXLAN-ni qabul qilish bir xil ma'muriy domenda 16 million `VXLAN` segmentlarining birgalikda yashashiga imkon beradi, zamonaviy ma'lumotlar markazlari va virtuallashtirilgan muhitlar uchun kengaytirilishi va moslashuvchanligini ta'minlaydi.

### Cisco Discovery Protocol
Cisco Discovery Protocol (CDP) Cisco kompaniyasining 2-qatlam tarmoq protokoli bo'lib, u Cisco qurilmalari, masalan, marshrutizatorlar, kalitlar va ko'priklar tomonidan bevosita ulangan boshqa Cisco qurilmalari haqida ma'lumot to'plash uchun foydalaniladi. Ushbu ma'lumotlardan tarmoq topologiyasini aniqlash va kuzatish hamda tarmoqni boshqarish va muammolarni bartaraf etishda foydalanish mumkin. Ushbu protokol odatda Cisco qurilmalarida yoqiladi, lekin agar kerak bo'lmasa yoki xavfsizlik sababli o'chirib qo'yish kerak bo'lsa, uni o'chirib qo'yish mumkin.
...
Ko'rsatilgan xabarda qurilma nomi, IP manzili, port nomi va yo'riqnoma funksiyasi kabi qurilmaning o'zi haqidagi ma'lumotlar, shuningdek qurilmaning operatsion tizimi va apparat platformasi haqidagi ma'lumotlar mavjud. Bundan tashqari, biz "CDPv2" ning birinchi qatorida "Cisco Discovery Protocol" bilan ishlayotganimizni ko'rishimiz mumkin.

Taqqoslash uchun biz Spanning Tree Protocol (`STP`) deb nomlangan boshqa protokolni ko'rishimiz mumkin. `STP` tarmoq protokoli boʻlib, u kalitlar oʻrtasida bir nechta ulanishga ega boʻlgan tarmoqda hech qanday halqa boʻlmasligini taʼminlaydi. Hech qanday halqalar yo'q va ma'lumotlar paketlarining tsiklda aylanishini va tarmoqni tiqilib qolishini oldini oladi.
...
Ushbu misolda biz “STP” xabari yuborilganligini ko'ramiz, unda ildiz kaliti, ildiz kalitining MAC manzili, xabar yuborilgan portning identifikatori va maksimal qarish vaqti, salom vaqti va oldinga o'tish kechikishi kabi boshqa konfiguratsiya parametrlari haqida ma'lumot mavjud.